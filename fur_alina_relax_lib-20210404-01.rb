...
define :far_start_background_sample do | num_beats_total, samp_hash |
  
  samp_hash.each do | samp_name, samp_ref |
    
    if samp_ref["samp"].class.to_s == "String" then
      samp = samps_dir + samp_ref["samp"]
    else
      samp = samp_ref["samp"]
    end
    ...
    puts "num_beats_total = " + num_beats_total.to_s
    puts "num_beats_sample = " + num_beats_sample.to_s
    puts "delay = " + samp_ref["delay"].to_s
    
    num_beats_sample -= samp_ref["delay"]
    
    if num_beats_sample > num_beats_total then
      puts "num_beats_sample > num_beats_total"
      n_finish = ( num_beats_total / num_beats_sample )
      n_repeat = 0
    else
      n_div = num_beats_total / num_beats_sample
    ...
    in_thread(name: "far_background_sound_" + samp_name) do
      ...
      sample samp, finish: n_finish, release: 2, amp: samp_ref["amp"], pan: samp_ref["pan"]
...
define :fur_alina_relax_background_sound do | num_beats_total |
  ...
  far_start_background_sample num_beats_total, samp_hash
